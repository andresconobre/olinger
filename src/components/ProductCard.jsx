...
const ProductCard = ({ data: { img, name, cardDetails, link } }) => {
  return (
    <a
      href={`/produtos/${link}`}
      className="p-4 flex flex-col gap-4 rounded-lg bg-white border border-blue-olinger-c4 shadow-card"
    >
      ...
      <div className="flex flex-col gap-[7px]">
        <h6 className="text-2xl font-semibold text-blue-olinger-c12">{name}</h6>
        {cardDetails && (
          <p className="text-sm text-gray-olinger-c9">{cardDetails}</p>
        )}
      </div>
      <Button to={`/produtos/${link}`} light>
        Detalhes
      </Button>
    </a>
  );
